...
// 새로운 게시글 추가 (POST)
router.post("/joy/insert", async (req, res) => {
  const paramTitle = req.body.title;
  const paramNickname = req.session.user
    ? req.session.user.nickname
    : "Anonymous"; // 작성자의 닉네임 (로그인 사용자 또는 'Anonymous')
    ...
    (err, rows) => {
      ...
      pool.query(
        ...
        (err, result) => {
          ...
          console.log(
            `좋아요 업데이트 성공: comment_no=${commentId}, newLikes=${
              currentLikes + 1
            }`
          );
...
router.get("/view_post/delete/:no", (req, res) => {
  ...
  pool.query(
    "SELECT nickname FROM board WHERE no = ?",
    [paramsNo],
    (error, results) => {
      if (error) {
